    ...
    def fetch_verdict(
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if attempt >= retries:
                    raise AIPanelClientError(
                        f"Claude request failed for {ticker}: {exc}",
                        model_name=MODEL_NAME,
                        retryable=True,
                    ) from exc
            ...
            if status == 429 or 500 <= status <= 599:
                if attempt >= retries:
                    raise AIPanelClientError(
                        f"Claude transient HTTP {status} for {ticker}.",
                        model_name=MODEL_NAME,
                        status_code=status,
                        retryable=True,
                    )
            ...
            if status >= 400:
                raise AIPanelClientError(
                    f"Claude HTTP {status} for {ticker}.",
                    model_name=MODEL_NAME,
                    status_code=status,
                    retryable=False,
                )
            ...
            try:
                ...
                raise AIPanelClientError(
                    f"Claude returned invalid JSON for {ticker}.",
                    model_name=MODEL_NAME,
                    retryable=False,
                ) from exc
        ...
        raise AIPanelClientError(
            f"Claude request exhausted retries for {ticker}.",
            model_name=MODEL_NAME,
            retryable=True,
        )
